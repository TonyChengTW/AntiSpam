...
if (not $findit) {
	...
	print "Location: spam_detail.pl?fixline=$fixline&chk_ip=$chk_ip&b_name=$b_name&h_name=$h_name&pop=$pop&chk_datetime=$chk_datetime&name=$name&custid=$custid&level=$level&username=$username&service=$service&logout=$logout&used=$used\n\n";
	exit; 
	}
