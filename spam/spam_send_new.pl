...
if ($level==1) {
	...
	$sql_up="insert into spam values('','$account','$chk_ip','$iptype','$ip_desc','$reason','$title','$newfileName','$the_from_date','$the_to_date',$level,'Y','$spam_userid',now())";
...
`echo "$sql_up" >> /tmp/spamlog.txt`;
	...
	$sql_up="update spam set ip='$chk_ip',iptype='$iptype',custdesc='$ip_desc',reason='$reason',title='$title',content='$newfileName',settime='$the_from_date',duetime='$the_to_date',level=$level,admin='',valid='Y',admin='$spam_userid',lastupdate=now() where account='$account'";
`echo "$sql_up" >> /tmp/spamlog.txt`;
}
        ...
        <tr align="center"> 
          <td width="420"> 
            <table border="0" width="420" align="center" cellpadding="5" bgcolor="#E1F0FF" bordercolor="#CCCCCC" cellspacing="3">
              ...
              <tr> 
                <td align="left" width="150" bgcolor="#E1F0FF"><font size="-1">檢舉信件內容(請使用右鍵另存新檔方式閱讀)
                  </font></td>
              ...
              <tr> 
                <td align="left" width="100%" colspan=3><a href="http://spam.apol.com.tw/spammail/'.$newfileName.'" target="_blank" >'.$newfileName.'</a></td>
              </tr>
